...
export function PreciseFooter({ palette: p, line = "Made in London." }) {
  return (
    <footer className="v4-grid" style={{ maxWidth: 1280, margin: "3rem auto 0", padding: "2rem 1.6rem 2.4rem", borderTop: `1px solid ${p.line}`, color: p.muted, fontFamily: "var(--f-ui)", fontSize: 12, display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: "1.6rem", letterSpacing: "0.02em" }}>
      ...
      <span className="mono" style={{ gridColumn: "10 / span 3", textAlign: "right" }}>© 2026 · last edit 2026.04.25</span>
    </footer>
  );
}
  ...
  return (
    <div className="v4-diagram-pair" style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "1rem", alignItems: "start" }}>
      <div style={{ position: "relative" }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: 300, background: `color-mix(in oklch, ${p.paper} 88%, white)`, border: `1px solid ${p.line}` }}>
          {engines.map(e => {
        ...
        <div className="mono" style={{ position: "absolute", left: 8, bottom: 8, fontSize: 10, color: hover === null ? p.muted : p.ink, padding: "2px 6px", background: `color-mix(in oklch, ${p.paper} 92%, white)`, border: `1px solid ${p.line}`, minWidth: 180 }}>
  ...
  return (
    <>
      ...
      <p style={{ fontSize: "0.85rem", color: p.muted, marginTop: "-0.5rem", lineHeight: 1.55 }}>
        <strong style={{ color: p.accent }}>2.</strong> The compression ratio matters: 384 raw audio descriptors → 16 latent dims. Anything that survives that bottleneck is doing real work.
      </p>
