  ...
  return (
    <>
      ...
      <p className="drop-cap" style={{ fontSize: "1.18rem", lineHeight: 1.72, margin: "0 0 1.2rem" }}>
        Across five units, the course assigns something like thirty readings. They disagree about almost everything on the surface — timelines, mechanisms, who should hold the wheel. But disagreement is the visible layer. Underneath it, the readings rest on a small number of shared assumptions that almost no author argues for, because almost no author notices they are assuming it. This essay names six. For each one I do three things: name the readings that depend on it, name the readings that quietly violate it, and ask what changes if the assumption breaks. This is a map, not a verdict.
      </p>
      ...
      <p>What changes if the assumption breaks? Almost everything downstream. The intelligence-explosion arguments need scalar capability to feed back on itself. If Chollet is right that capability is situated and jagged, then "AGI by 2027" is not so much wrong as ill-typed — you cannot reach a point on an axis that doesn't exist.<span className="sidenote-number" style={{ color: p.accent }}>1</span>
      ...
      <p>What breaks: the chip-control theory of governance. Export controls, SL5 weight-security, compute treaties — all assume that gating compute gates outcomes. If returns per unit of compute are flat, the most legible policy instrument in the course is aimed slightly off-target.</p>
      ...
      <p>What breaks if defence is <em>not</em> enough: you can win every defensive engagement and still lose, because nothing pulls the system toward a good equilibrium. The "withstand" layer feels suspiciously agreeable precisely because it is the most open to interpretation, and therefore the least binding. That is the signature of a corpus that has worked hard on the brakes and barely touched the steering.</p>
